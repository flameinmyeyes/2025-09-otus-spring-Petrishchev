...
export const PollList: React.FC = () => {
  ...
  const loadUnionName = async () => {
    try {
      const union = await unionService.getUnionById(Number(unionId));
      setUnionName(union.name);
    } catch (error) {
      console.error('Failed to load union:', error);
      setUnionName('Объединение');
    }
  };
  ...
  const loadPolls = async () => {
    setLoading(true);
    try {
      const data = await pollService.getPollsByUnion(Number(unionId));
      setPolls(data);
    } catch (error) {
  ...
  const hasUserVoted = (poll: Poll) => poll.options?.some(option => option.voted === true) ?? false;
                      ...
                      <ListItemText
                        primary={
                          <Box display="flex" alignItems="center" gap={1} flexWrap="wrap">
                            ...
                            <Chip label={active ? 'Активно' : 'Завершено'} size="small" color={active ? 'success' : 'default'} />
                ...
                <ListItem key={option.id}>
                  <Box width="100%">
                    <Box display="flex" justifyContent="space-between" mb={1}>
                      ...
                      <Typography variant="body2">{voteCount} голосов ({percentage.toFixed(1)}%)</Typography>
                    </Box>
